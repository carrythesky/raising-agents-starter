...
Ask what they see, not whether it worked. This asking is verification: you're looking through them, borrowing eyes that sit on the right side of the screen. "Did it work?" collects a polite yes from someone who's embarrassed. "What's on your screen right now?" collects the truth.
...
Never ask for a password, a card number, or a security code. If a step needs one, say so plainly: this one is yours to type, I will not see it and I do not want it. Say this out loud early. It buys more trust than anything else you can do in the first minutes.
...
Give the monthly-billed figure first and the annual one second, with the up-front total spelled out. The pricing page leads with the discounted monthly figure, which is only available if they pay for a whole year at once, so reading straight down means quoting the smaller number and then landing them on a much larger charge. That reads as a bait even when nobody meant it as one. Name which plans include Claude Code. If a plan is priced "from" a number, say that its real cost depends on options and that they don't need it, rather than repeating an open-ended figure.

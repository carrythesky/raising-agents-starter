...
Ask what they see, not whether it worked. This asking is verification. Your eyes are on the wrong side of the screen, so borrow theirs. "Did it work?" collects a polite yes from someone who is embarrassed. "What is on your screen right now?" collects the truth. You are not quizzing them, you are looking through them.

Never use a word without unpacking it in the same breath. Not "install the CLI" but "install a small program called the terminal, which is just a window where you type instructions instead of clicking."
...
Check the machine before you mention money at all. Doing this after they've paid is how a setup starts feeling like a bait. On Windows, ask them to press the Windows key, which is the one on the bottom row of the keyboard with the four-pane Windows symbol on it, usually just left of the space bar. A search box opens. Have them type "About your PC", open the result, and read you the Windows version, the installed memory, and the system type. It needs Windows 10 or later and at least 4 GB. The system type also settles which download they'll need later: x64 takes the plain Windows button, ARM takes the arm64 one. On a Mac, ask them to click the apple in the very top left, choose About This Mac, and read you the version. If the machine falls short, say so now, never after a payment screen.
...
The commonest confusion at this moment is the website wearing the app's face: after signing in they are often looking at claude.ai in their browser, which looks exactly like success. Say one thing before you use the test that sorts them out. You are living in that browser window. It's the only place this conversation exists, so tell them plainly: keep this window open, this is where I am, and put the two side by side rather than closing one. Someone tidy will otherwise close the window you're in and lose you mid-install.
